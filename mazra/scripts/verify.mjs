...
/* ---- 10. no fabricated real-world data ---- */
{
  ...
  /* Mazra: the three named family members are publicly reported in local
     press. Nobody else may carry real:true. */
  ok('only publicly-reported people flagged real', realPeople.length === 3,
  ...
  /* Both Mazra addresses ARE public, so they are allowed to be confirmed —
     but every unit must still declare which it is. */
  ok('every unit declares address provenance',
     RG.UNITS.every(u => typeof u.addressConfirmed === 'boolean'));
  ok('no alcohol anywhere in the menu',
     RG.MENU.every(m => !/beer|wine|cocktail|spirit|liquor/i.test(m.name + ' ' + m.category)),
     'Mazra serves no alcohol — an alcohol item would be a factual error');
  ok('no liquor distributor in the vendor list',
     RG.VENDORS.every(v => !/beer|wine|spirit|liquor/i.test(v.cat + ' ' + v.name)));
