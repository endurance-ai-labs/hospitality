    ...
    ALL.forEach(function (uid) {
      ...
      if (cogs.variancePct > 0.035 && cogs.variance > 2500) {
        ...
        flags.push({
          ...
          link: '/hospitality/mazra/cogs?unit=' + uid, sources: ['Toast', 'R365']
        });
      }
      ...
      if (laborDelta > 0.010) {
        flags.push({
          ...
          detail: 'Overtime ' + fmt$(cur.otCost) + ', break premiums ' + fmt$(cur.breakPremiums) +
                  ', SPLH ' + fmt$(cur.splh) + '.',
          link: '/hospitality/mazra/labor?unit=' + uid, sources: ['7shifts', 'Toast']
        });
      }
      if (cur.comps / Math.max(1, cur.grossSales) > 0.014) {
        flags.push({
          ...
          link: '/hospitality/mazra/cash?unit=' + uid, sources: [u.pos]
        });
      }
      if (u.pctRentBreak) {
        ...
        if (prox > 0.62) {
          flags.push({
            ...
            link: '/hospitality/mazra/leases?unit=' + uid, sources: ['Lease abstract', 'Toast']
          });
        }
      }
      var py = PRIOR_YEAR ? M.unit[uid][PRIOR_YEAR.key] : null;
        ...
        if (comp < 0) {
          flags.push({
            ...
            detail: 'Net sales ' + fmt$(cur.netSales) + ' against ' + fmt$(py.netSales) +
                    ...
                    ' vs. ' + fmtNum(py.covers) + '.',
            link: '/hospitality/mazra/sales?unit=' + uid, sources: [u.pos]
          });
        }
      }
    });
