...
(function (global) {
  ...
  var DOW_SHAPE = {
    /* Open seven days. Fast-casual Levantine: strong weekday lunch, big
       Friday and Saturday, softer Sunday. */
    mazra: [0.88, 0.92, 0.96, 1.04, 1.30, 1.36, 1.02]
  };

  /* seasonal amplitude and phase (day-of-year of the annual peak) */
  var SEASON = {
    mazra: { amp: 0.078, peak: 214 }     /* mild summer peak, grill-led */
  };

  /* daypart mix by brand */
  var DAYPART_MIX = {
    /* A lunch business first — no bar means no late-night trade to speak of. */
    mazra: { lunch: 0.38, happy: 0.14, dinner: 0.42, late: 0.06 }
  };
  ...
  function eventFactor(unit, day) {
    /* weekend spikes */
    if (day.dow >= 4 && R.chance('ev:' + unit.id + day.iso, 0.10)) return 1.09;
  ...
  var dayPmix = R.memo(function (unitId, iso) {
    ...
    var menu = RG.menuForUnit ? RG.menuForUnit(unit) : RG.menuFor(unit.brand);
  ...
  function adjustments(unit, day, gross) {
    ...
    var base = 0.019;
    /* Redwood City is the newer room with the newer team — comps and voids
       run hotter there while it finds its feet. */
    if (unit.id === 'mz-rwc') base = 0.036;
    var promoDay = 0;
